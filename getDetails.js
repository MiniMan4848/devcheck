...
export async function getDetails(url) {
  ...
  const name = await getData(".align-end > div:nth-child(1) > div:nth-child(1)");
  const wallet = await getData(".sm\\:text-sm");
  const followers = await getData(".align-end > div:nth-child(1) > div:nth-child(2) > div:nth-child(1)");
  ...
  var nextButtonClassName = await (await (await page.$("button.text-sm:nth-child(3)")).getProperty("className")).jsonValue();

  // Loop while the button is enabled
  if (nextButtonClassName === "text-sm text-slate-50 hover:font-bold hover:bg-transparent hover:text-slate-50") {
    while (nextButtonClassName === "text-sm text-slate-50 hover:font-bold hover:bg-transparent hover:text-slate-50") {
      
      await page.click("button.text-sm:nth-child(3)");
      // wait for page to load
      await page.waitForSelector(".max-w-\\[400px\\]");
      var pageCount = await page.$$eval(".max-w-\\[400px\\] > a", (as) => as.length);
